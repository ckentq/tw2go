...
class ProfileField extends CActiveRecord
{
	...
	public function scopes()
    {
        return array(
			...
			'forShopOwner'=>array(
                'condition'=>'required='.self::REQUIRED_NO_NOT_SHOW_REG_OWNER,
                'order'=>'position',
            ),
        );
    }
	...
	public static function itemAlias($type,$code=NULL) {
		$_items = array(
			...
			'required' => array(
				...
				self::REQUIRED_NO_NOT_SHOW_REG_OWNER => UserModule::t('only shop onwer need'),
			),
			'visible' => array(
				...
				self::VISIBLE_ONLY_SHOP => UserModule::t('for shop owner only'),
			),
		);
		if (isset($code))
